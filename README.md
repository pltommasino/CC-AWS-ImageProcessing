...
Open **the exact folder that contains the HTML file** you want to expose through HTTPS on localhost.  
Live Server reads its configuration only from the root of the opened folder, so this step is crucial.
2. Create the `https` directory
...
```
This command generates a **self-signed certificate** using the private key from step 3 and sets a validity of **10 years**.  
You will be asked for several pieces of information (Country, City, Organization, etc.).  
You can simply press **Enter** for all fields.
5. Create the `.vscode` directory
...
6. Create the `settings.json` file
Run:
```bash
touch settings.json
```
7. Configure Live Server to use HTTPS
Open the newly created `settings.json` file and insert:
```bash
{
...
8. Run Live Server with HTTPS
Open your HTML file in VS Code, right-click **Open with Live Server**. Your project will now be served over **HTTPS**.